...
def test_get_feed_with_channels(client, db_engine, tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CHANNELS_FILE", str(tmp_path / "channels.json"))
    (tmp_path / "channels.json").write_text('["https://www.youtube.com/@GODofIT"]')
...
def test_get_feed_channel_detail(client, db_engine):
    ...
    resp = client.get("/api/feed/GODofIT")
    assert resp.status_code == 200
    data = resp.json()
...
def test_get_feed_channel_case_insensitive(client, db_engine):
    _, connection = db_engine
    _add_video(connection)

    resp = client.get("/api/feed/godofIT")  # different case
    assert resp.status_code == 200
    assert resp.json()["channel_name"] == "GODofIT"


def test_get_feed_channel_not_found(client):
    resp = client.get("/api/feed/UnknownChannel")
    assert resp.status_code == 200
    data = resp.json()
    assert data["videos"] == []
